...
def main():
    ...
    # compositor_optimizers = [create_compositor_optimizers(models=models, config=configs, model_idx=i) for i in range(configs["num_models"])]
    lr_schedulers = create_lr_schedulers(optimizers, config=configs)
    # lr_compositor_schedulers = [create_lr_schedulers(optimizer, config=configs) for optimizer in compositor_optimizers]
    print(f"Number of ensemble models : {configs['num_models']}")
