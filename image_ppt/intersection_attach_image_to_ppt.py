...
if __name__ == '__main__':
    ...
    for DOMAIN in domains:
        ...
        CosMo_parent_path = f'/home/jaehyun98/git/uncertain_retrieval/visualize_result/CosMo_eval/{DOMAIN}/intersection/'
        MGUR_parent_path = f'/home/jaehyun98/git/uncertain_retrieval/visualize_result/MGUR_eval/{DOMAIN}/intersection/'
        ...
        all_indexes = os.listdir(CosMo_parent_path)
        ...
        for cur_index in all_indexes:
            ...
            cosmo_cur_path = os.path.join(CosMo_parent_path, cur_index)
            mgur_cur_path = os.path.join(MGUR_parent_path, cur_index)

            source_image_path = os.path.join(cosmo_cur_path, 'source_image')
            answer_image_path = os.path.join(cosmo_cur_path, 'answer_image')
            # retrieved_proper_image_path = os.path.join(cur_path, 'retrieved_proper_images')
            retrieved_image_path = os.path.join(cosmo_cur_path, 'intersection_retrieved_images')
            mgur_retrieved_image_path = os.path.join(mgur_cur_path, 'intersection_retrieved_images')
            ...
            mgur_retrieved_images = os.listdir(mgur_retrieved_image_path)
            ...
            textbox1 = slide.shapes.add_textbox(tb1_left, tb1_top, tb1_width, tb1_height)
            tf1 = textbox1.text_frame
            tf1.text = "Retrieved Images with CosMo"

            # Retrieved Images
            for idx, cur_retrieved_proper_image in enumerate(retrieved_images):
                left = Inches(idx)  # 1 inch from the left of the slide
                top = Inches(3.1)  # 1 inch from the top of the slide
                height = Inches(1)  # Height of the image
                pic = slide.shapes.add_picture(os.path.join(retrieved_image_path, cur_retrieved_proper_image), left, top, height=height)
            ...
            textbox1 = slide.shapes.add_textbox(tb1_left, tb1_top, tb1_width, tb1_height)
            tf1 = textbox1.text_frame
            tf1.text = "Retrieved Images with MGUR"

            # Retrieved Images
            for idx, cur_retrieved_image in enumerate(mgur_retrieved_images):
                left = Inches(idx)  # 1 inch from the left of the slide
                top = Inches(5.1)  # 1 inch from the top of the slide
                height = Inches(1)  # Height of the image
                pic = slide.shapes.add_picture(os.path.join(mgur_retrieved_image_path, cur_retrieved_image), left, top, height=height)
        ...
        prs.save(f'CosMo_MGUR_intersection_{dataset}_{DOMAIN}_analysis.pptx')